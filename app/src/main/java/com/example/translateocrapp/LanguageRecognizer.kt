...
class LanguageRecognizer {
    ...
    fun recognizeLanguage(ocrMap: Map<Rect, Text.TextBlock>): String {
        ...
        for ((rect, textBlock) in ocrMap) {
            ...
            fullExtractedText.append(text).append(" ") // Append text with a space
        ...
        // Log the full extracted text at once
        Log.d("ExtractedText", "Full OCR Extracted Text: $fullExtractedText")
    ...
    fun extractedText( languageText: StringBuilder): StringBuilder {

        return languageText
    }

    fun languageCode(languageCode: String): String {
        return languageCode
    }
